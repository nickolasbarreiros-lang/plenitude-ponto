...
function card(r){const actionable=['pendente','conflitante'].includes(r.status);return `<article class="panel contingency-card">
<div class="contingency-card-head"><div><small>${r.funcionario_nome} · ${r.matricula}</small><h3>${label(r.tipo)} — ${fmt(r.ocorrido_em_dispositivo)}</h3><p>Dispositivo: ${r.dispositivo_nome} · sincronizado em ${fmt(r.sincronizado_em)}</p></div><span class="request-status ${r.status==='aprovado'?'aprovada':r.status==='rejeitado'?'rejeitada':'pendente'}">${r.status}</span></div>
