...
async function queryAll(table,select='*',apply=q=>q){let all=[],from=0;const step=1000;while(true){let q=client.from(table).select(select).range(from,from+step-1);q=apply(q);const {data,error}=await q;if(error)throw error;all.push(...(data||[]));if(!data||data.length<step)break;from+=step}return all}
async function loadData(){
 const {start,end}=period();$('bk-progress').textContent='Consultando o Supabase...';
 const [profile,funcionarios,jornadas,marcacoes,ocorrencias,ajustes,auditoria]=await Promise.all([
  window.PlenitudeDB.profile(),
  queryAll('funcionarios','*',q=>q.order('nome')),
  queryAll('jornadas','*',q=>q.order('funcionario_id').order('dia_semana')),
  queryAll('marcacoes','*',q=>q.gte('data_local',start).lte('data_local',end).order('registrado_em')),
  queryAll('ocorrencias','*',q=>q.lte('data_inicio',end).gte('data_fim',start).order('data_inicio')),
  queryAll('solicitacoes_ajuste','*',q=>q.gte('data_marcacao',start).lte('data_marcacao',end).order('criado_em')),
  queryAll('logs_auditoria','*',q=>q.gte('criado_em',`${start}T00:00:00-03:00`).lte('criado_em',`${end}T23:59:59-03:00`).order('criado_em'))
 ]);
 const banco=[];
