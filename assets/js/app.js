...
async function initFuncionarios(){
  ...
  try{
    ...
    document.getElementById('salvar-pin').onclick=async()=>{if(!DB_STATE.employee)return toast('Salve o funcionário antes de definir o PIN.','warn');const pin=document.getElementById('func-pin').value,confirmPin=document.getElementById('func-pin-confirm').value;if(!/^\d{4}$/.test(pin))return toast('O PIN deve conter exatamente 4 números.','warn');if(pin!==confirmPin)return toast('A confirmação do PIN não coincide.','warn');const b=document.getElementById('salvar-pin');b.disabled=true;b.textContent='Salvando...';try{const result=await window.PlenitudeDB.defineEmployeePin(DB_STATE.employee.id,pin,document.getElementById('exigir-troca-pin').checked,document.getElementById('acesso-pin-ativo').checked);DB_STATE.employee={...DB_STATE.employee,...result,pin_configurado:true};document.getElementById('func-pin').value='';document.getElementById('func-pin-confirm').value='';await renderEmployee(DB_STATE.employee);toast(`PIN definido com sucesso para a matrícula ${result.matricula}.`)}catch(error){toast(errorText(error),'warn')}finally{b.disabled=false;b.textContent='Definir / alterar PIN'}};
