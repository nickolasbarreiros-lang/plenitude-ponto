...
async function initFuncionarios(){
  ...
  try{
    ...
    document.getElementById('vincular-acesso').onclick=async()=>{const email=document.getElementById('func-email').value.trim();if(!DB_STATE.employee)return toast('Salve o funcionário antes de vincular a conta.','warn');if(!email)return toast('Informe o e-mail criado no Supabase Auth.','warn');const b=document.getElementById('vincular-acesso');b.disabled=true;b.textContent='Vinculando...';try{const saved=await window.PlenitudeDB.linkEmployeeAccess(DB_STATE.employee.id,email);DB_STATE.employee=saved;await renderEmployee(saved);toast('Conta de acesso vinculada.')}catch(error){toast(errorText(error),'warn')}finally{b.disabled=false;b.textContent='Vincular conta de acesso'}};
    document.getElementById('baixar-qr').onclick=downloadEmployeeQr;
    document.getElementById('imprimir-qr').onclick=printEmployeeQr;
  }catch(error){toast(errorText(error),'warn');console.error(error)}
}
function fillEmployeeForm(f){['nome','cpf','cargo','admissao'].forEach(k=>{const el=document.getElementById(k);if(el)el.value=f?(k==='admissao'?f.data_admissao||'':f[k]||''):''});const email=document.getElementById('func-email');if(email)email.value=f?.email_acesso||'';const matricula=document.getElementById('matricula');if(matricula)matricula.value=f?.matricula||'';const status=document.getElementById('func-status');if(status)status.value=f?.status||'ativo';window.__employeePhotoData=null;window.__employeePhotoFileSelected=false}
...
async function renderEmployee(f){document.getElementById('func-nome').textContent=f?.nome||'Nenhum funcionário cadastrado';const status=f?.status||'ativo',badge=document.getElementById('func-status-badge');if(badge){badge.className=`employee-status ${status}`;badge.innerHTML=`<i></i> ${employeeStatusLabel(status)}`};document.getElementById('func-status-top').textContent=f?`1 funcionário ${status==='ativo'?'ativo':'cadastrado'}`:'Nenhum funcionário';renderAvatar(employeePhoto(f),f?.nome);document.getElementById('foto-status').textContent=f?.foto_url?'Foto armazenada com segurança no Supabase.':'Nenhuma foto cadastrada.';document.getElementById('acesso-status').textContent=f?.auth_user_id?'Conta vinculada e pronta para login.':'Ainda sem conta vinculada.';document.getElementById('func-detalhes').innerHTML=f?`<span><strong>Cargo:</strong> ${f.cargo||'—'}</span><span><strong>CPF:</strong> ${f.cpf||'—'}</span><span><strong>Matrícula:</strong> ${f.matricula||'—'}</span><span><strong>Admissão:</strong> ${f.data_admissao?new Intl.DateTimeFormat('pt-BR').format(new Date(f.data_admissao+'T12:00:00')):'—'}</span><span><strong>Carga semanal:</strong> ${fmtMinutes(f.carga_semanal_minutos||2640)}</span><span><strong>Acesso:</strong> ${f.auth_user_id?'Vinculado':'Pendente'}</span>`:'<span>Preencha o formulário para cadastrar o primeiro funcionário.</span>';const code=f?.codigo_qr||'';document.getElementById('func-qr-code').textContent=code||'Será gerado ao salvar';const qr=document.getElementById('func-qrcode');if(qr){qr.innerHTML='';if(code&&window.QRCode)new QRCode(qr,{text:`${location.origin}${location.pathname.replace(/funcionarios\.html$/,'ponto.html')}?codigo=${encodeURIComponent(code)}`,width:112,height:112,colorDark:'#2a2526',colorLight:'#ffffff',correctLevel:QRCode.CorrectLevel.M})}}
