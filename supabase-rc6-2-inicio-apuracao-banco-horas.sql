-- ============================================================
-- PLENITUDE PONTO RC6.2
-- INÍCIO REAL DA APURAÇÃO DO BANCO DE HORAS
-- ============================================================
--
-- Corrige o débito retroativo de julho/2026.
--
-- Roseli começou a utilizar o Plenitude Ponto em 03/08/2026.
-- Dias anteriores a essa data deixam de ser classificados como falta.
--
-- O SQL:
--   1. cria data_inicio_apuracao em funcionarios;
--   2. define Roseli (matrícula 001) = 03/08/2026;
--   3. corrige o motor do banco de horas;
--   4. apaga snapshots antigos calculados incorretamente;
--   5. reconstrói competências fechadas com a regra corrigida.
--
-- Não altera nem apaga marcações reais.

begin;

alter table public.funcionarios
  add column if not exists data_inicio_apuracao date;

alter table public.funcionarios
  alter column data_inicio_apuracao set default current_date;

comment on column public.funcionarios.data_inicio_apuracao is
'Primeiro dia que participa da apuração do banco de horas. Datas anteriores são ignoradas.';

update public.funcionarios
set data_inicio_apuracao='2026-08-03'::date
where matricula='001';
...
-- Remove snapshots produzidos pela regra antiga somente para os funcionários
-- que agora possuem data de início de apuração.
delete from public.banco_horas_competencias bh
using public.funcionarios f
where bh.funcionario_id=f.id
  and f.data_inicio_apuracao is not null;

-- Reconstrói os fechamentos existentes com o motor corrigido.
do $$
declare
  v_fechamento record;
begin
